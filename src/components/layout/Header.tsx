...
export const Header = () => {
  ...
  const { favoritedIds, fetchWatchlist } = useWatchlistStore();
      ...
      <div className="container-custom">
        <div className="flex items-center justify-between h-16">
          ...
          <div className="flex items-center gap-4">
            <Link
              to="/marketplace"
              className="relative flex items-center p-2 text-neutral-600 hover:text-blue-600"
              aria-label="Watchlist"
            >
              <Heart size={20} />
              {favoritedIds.size > 0 && (
                <span className="absolute -top-1 -right-1 flex items-center justify-center min-w-[18px] h-[18px] px-1 rounded-full bg-red-500 text-white text-xs font-medium">
                  {favoritedIds.size}
                </span>
              )}
            </Link>
